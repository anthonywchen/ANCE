...
def get_latest_checkpoint(args):
    ...
    if len(checkpoint_nums) > 0:
        return os.path.join(
            args.training_dir, "checkpoint-" + str(max(checkpoint_nums))
        ), max(checkpoint_nums)
    return args.init_model_dir, 0
...
# streaming inference
def StreamInferenceDoc(
    args, model, fn, prefix, f, is_query_inference=True, load_cache=False
):
...
def generate_new_ann(
    args, output_num, checkpoint_path, preloaded_data, latest_step_num
):
    ...
    with dev_query_cache_trivia as emb:
        dev_query_embedding_trivia, dev_query_embedding2id_trivia = StreamInferenceDoc(
            args,
            model,
            GetProcessingFn(args, query=True),
            "dev_query_" + str(latest_step_num) + "_",
    ...
    if is_first_worker():
        ...
        # measure ANN mrr
        _, dev_I = cpu_index.search(
            dev_query_embedding, 100
        ...
        # measure ANN mrr
        _, dev_I = cpu_index.search(
            dev_query_embedding_trivia, 100
        )  # I: [number of queries, topk]
        top_k_hits_trivia = validate(
            passage_text,
            test_answers_trivia,
            dev_I,
        ...
        logger.info(
            "Start searching for query embedding with length %d", len(query_embedding)
        )
        ...
        logger.info(
            "Effective qid length %d, search result length %d",
            len(effective_q_id),
            I.shape[0],
        )
        ...
        logger.info(
            "Done generating negative passages, output length %d",
        ...
        with open(ndcg_output_path, "w") as f:
            json.dump(
                {
                    "top20": top_k_hits[19],
                    "top100": top_k_hits[99],
                    "top20_trivia": top_k_hits_trivia[19],
                    "top100_trivia": top_k_hits_trivia[99],
...
def ann_data_gen(args):
    ...
    while args.end_output_num == -1 or output_num <= args.end_output_num:
        ...
        if next_checkpoint == last_checkpoint:
            ...
            generate_new_ann(
                args, output_num, next_checkpoint, preloaded_data, latest_step_num
            )
